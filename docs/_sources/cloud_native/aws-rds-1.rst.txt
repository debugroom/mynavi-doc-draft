...
.. _section-cloud-native-rds-label1-1:
...
RDSでは2019年3月現在、以下のデータベースエンジンを選択可能です。
...
RDS構築時は、複数のアベイラビリティゾーンをまたぎ、リードレプリカを構築できるオプションがあり、
...
これでRDS DBインスタンスが作成できました。次回はこのデータベースにテーブルを作成し、アクセスするSpringアプリケーションを実装していきます。
