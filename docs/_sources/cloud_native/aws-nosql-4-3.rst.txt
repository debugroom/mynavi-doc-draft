...
また、次回以降、AWS ECSを使って複数のコンテナ上で、当アプリケーションを実行しますが、画面から指定するcontainerGroupというパラメータで
アプリケーションロードバランサーが実行するコンテナを振り分けて指定できるように、セッションを保存するURLにはパスパターンにパラメータを含めておきましょう。
