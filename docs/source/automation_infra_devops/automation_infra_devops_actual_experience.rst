...
拙著 `【連載】ソフトウェア開発自動化入門 基盤・デプロイ自動化 <https://news.mynavi.jp/itsearch/article/devsoft/4285>`_ で近年の動向である
マイクロサービスアーキテクチャアプリケーションの基盤構築自動化・継続的インテグレーション・デプロイ(CI/CD)自動化の実現例を紹介しました。
本連載では、この例に従って、アプリケーションの実装・CI/CD自動化・基盤自動化を実際に構築してきます。本連載では、以下の様なステップで進めていきます。
...
マイクロサービスアーキテクチャでのアプリケーション構成と構築
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

|br|

マイクロサービスアーキテクチャでは従来のモノリシックなアプリケーション(全ての機能が一つのアプリケーションパッケージにまとまっている形)に比べ、
アプリケーション構成はもとより、チーム間での開発体制からリリースの単位までこれまでとは異なる形で行うことを要求されます。

一般にマイクロサービスアーキテクチャにすることで、以下の様なメリットがあります。
...
従って、上記のメリットを実現するために、サービスの分割・アプリケーションの作成・実行単位、プロジェクト構成、開発のスコープやスパン・体制、リリースの最適な単位を、個々のアプリケーションごとに検討する必要があります。
当然、唯一解はないのですが、以下の様なアプリケーションアーキテクチャが１つの構成例となるでしょう。
...
* アプリケーションはフロントエンド(Backend For Frontend: BFFとも呼ばれる)とバックエンドのサービスに分割し、各サービスはAWS ECSコンテナ上にデプロイされるものとする。
...
各マイクロサービスはDockerをベースとするECSコンテナ単位で作成します。サービスの単位は主に以下の基本原則・考え方により、適切な粒度や分割単位を決定します。
...
また、セキュリティの観点やサービスのスケーラビリティ、ポータビリティ・再利用性を考慮し、サービスを論理的にフロントエンド(BFF)とバックエンドに分割していますが、それぞれの役割は以下の通りです。

.. list-table:: フロントエンド(BFF)とバックエンドの役割
   :widths: 5, 5
...
上記の構成で実際にアプリケーションを構築する場合は、拙著 「 :ref:`section-cloud-native-ecs-1st-label` 」を参考に進めてみてください。
セッションキャッシュやバックエンドのデータベース構築については盛り込んでいませんが、本連載の趣旨からは問題ありませんし、各々構築方法は、連載「AWSで作るクラウドネイティブアプリケーションの基本」の今後で触れたいと思います。

|br|

次回は、このアプリケーション環境を前提とした継続的インテグレーション(CI)をAWS CodeBuildを使って実現してみます。
